...
get_preliminary_results <- function(use.saved = FALSE) {
    ...
    if (use.saved) {
        ...
        ## Add delay
        tern$delay <- as.numeric(with(
            tern,
            difftime(incident__date_of_arrival, incident__date_of_injury, units = "hours")
        ))
        ...
        relevant.variables <- c(
            ...
            "patinfo__pt_gender",
            "incident__moi",
            "interventions__admitted",
            "in.hospital.mortality"
        )
        rates.relevant.missing.data <- rates.missing.data[relevant.variables]
        ...
        all.data$eligible <- with(
            all.data,
            (age >= 15 & !is.na(age)) & (admitted & !is.na(admitted)) & (delay < 48 & !is.na(delay))
        )
        ...
        preliminary.results$rate.in.hospital.mortality <- mean(
            eligible.data$in.hospital.mortality,
            na.rm = TRUE
        )
        preliminary.results$rate.in.hospital.mortality.males <- mean(
            eligible.data$in.hospital.mortality[eligible.data$sex == "Male"],
            na.rm = TRUE
        )
        preliminary.results$rate.in.hospital.mortality.females <- mean(
            eligible.data$in.hospital.mortality[eligible.data$sex == "Female"],
            na.rm = TRUE
        )
