...
We select patients for the nested staircase outcomes separately from cluster randomisation, by stratified random sampling by shift (see Nested staircase design and secondary outcomes).
...
At the cluster level we treat late or early transition relative to the scheduled date, incomplete delivery of ATLS^®^ training, and cluster dropout as protocol or adherence issues (see Adherence and protocol deviations) rather than as outcome-specific intercurrent events. We still define exposure for these clusters as specified under @sec-analysis-sets.
...
We define the treatment effects in this plan for the trial's eligible clusters and eligible patient participants (see @sec-eligibility-criteria).
...
We will treat clusters, and the observations within them, as exposed to the intervention after the cluster's scheduled transition date. We will include all data except the transition phases. A patient's intervention condition depends on the period in which they were admitted. We will also define exposure by the actual transition date in a sensitivity analysis (see Sensitivity analyses).

We specify the analysis set for each analysis. It is the target population for that outcome (see Population), restricted to patients with observed outcome data and to any further outcome-specific constraints in @tbl-outcomes-summary. For adjusted analyses under the available-case approach we restrict the analysis set to patients with complete covariate data (see @sec-missing-data).
...
@tbl-outcomes-summary lists the comparative effect measures by outcome; we estimate them from the mixed-effects models in @sec-analysis-methods. The effects are cluster-specific unless otherwise stated. The types of effect measures are as follows:
...
We will analyse the primary and secondary outcomes with mixed-effects regression models, matching the link function and outcome distribution to each outcome's data type.
...
We will fit the mixed-effects models by maximum likelihood, using a Laplace approximation to integrate out the random effects.
...
We will use model diagnostics to check distributional assumptions, convergence, and, where relevant, the intercurrent-event strategy assumptions noted under Intercurrent events.
...
### Exposure defined by the actual transition date
