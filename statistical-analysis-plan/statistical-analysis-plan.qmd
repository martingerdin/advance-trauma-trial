...
At the cluster level, adherence means that ATLS® training is delivered to the intended physicians during the scheduled transition period for that cluster’s randomised sequence, and that the cluster does not implement ATLS® training before its scheduled transition. Cluster-level adherence will be assessed using training-delivery records (including numbers of physicians trained in each hospital and timing of training relative to the scheduled transition). We will summarise cluster-level adherence descriptively, including the number and proportion of clusters that completed training as scheduled and the reasons for any non-adherence to the randomisation schedule. Adherence to the randomisation schedule is also reviewed at the interim analysis (see Interim analysis).

At the individual level, exposure to the intervention is defined by the cluster’s scheduled transition date, as specified under Analysis sets: patients are classified according to the period of admission, transition months are excluded, and the primary analysis includes eligible patients with available outcome data irrespective of whether care was delivered by a trained physician. Individual-level receipt of care from ATLS®-trained physicians will be monitored descriptively where available, but will not redefine the primary analysis set. If a cluster’s actual transition date differs from the scheduled date by more than four weeks, exposure will be redefined using the actual transition date in a sensitivity analysis (see Analysis sets).

Process adherence to ATLS® principles during initial resuscitation (the 14-item checklist) is a nested-staircase secondary outcome and is defined and analysed in the Outcomes and Analysis of secondary outcomes sections; it is not used to exclude patients from the primary analysis.
...
<!-- Should include:
- Shell figure visualizing information to be included in the cluster-specific CONSORT flow diagram (e.g., recruitment data, study withdrawal), to include:
  - For each arm (or sequence, or period), the numbers of clusters and individuals that were randomly assigned, received intended intervention, and were analysed for the primary outcome
  - For each arm (or sequence or period), losses and exclusions (for both clusters and individuals) after randomisation, together with reasons
-->
...
The treatment effects estimated in this plan are defined for the trial’s eligible clusters and eligible patient participants, as specified under Eligibility criteria.
...
Operational membership of each analysis (available outcome data, covariate completeness for adjusted analyses, and scheduled exposure rules) is specified under Analysis sets.
...
<!-- Should include:
- Details of how each outcome (both cluster-level and individual-level) will be summarized by arm (or sequence, period), for each data type (e.g., binary, continuous), including how the prevalence of missing data will be reported; and if any averaging will be by cluster or individual
- For each outcome data type (e.g., binary, continuous) what effect measures will be used to 2describe the treatment effect (e.g., odds ratio, mean difference); and if any averaging will be by cluster or individual
- Shell table visualising how primary and secondary outcomes (both cluster-level and individual-level), will be summarized and compared by arm (or sequence or period)
-->

<!-- sap-revision-todo J1: key vs all descriptive outcome shell tables (domain-level EQ-5D-5L/WHODAS in supplementary). -->
Summaries of primary and secondary outcomes will be presented in the results section, stratified by before and after ATLS training and overall, using the same summary statistics as the patient characteristics tables. Missing values will be reported for each outcome. We will present key outcome summaries in the results section, and present all outcome summaries as supplementary tables. See @tbl-outcomes-descriptive for key outcomes and @tbl-outcomes-descriptive-all for all outcomes.
