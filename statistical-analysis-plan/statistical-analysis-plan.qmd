...
If 10% or more of participants have a missing primary outcome, we will use multiple imputation by chained equations (MICE) under a missing-at-random assumption, imputing the primary outcome, secondary outcomes as required for analysis, and the covariates included in the fully adjusted model, with a minimum of 20 imputations. Unadjusted and adjusted analyses will then be conducted using the imputed datasets. We will also present the corresponding complete-case analyses for comparison. Secondary outcomes will not be imputed under the available-case approach.
