...
We have designed and will analyse the trial within a superiority framework, with ATLS® training compared with standard care for the primary and secondary outcomes. The primary statistical target is a cluster-specific (conditional) intervention effect among eligible patient participants. It is not a marginal (population-averaged) effect, and it is not an equal-weight average across clusters. 
...
Random selection of patients for nested staircase outcome collection is separate from cluster randomisation and is described under @sec-nested-staircase-design-and-secondary-outcomes (stratified random sampling by shift).
...
## Type 1 error control {#sec-type-1-error-control}
...
## Study withdrawal and follow-up {#sec-study-withdrawal-and-follow-up}
...
Comparative effect measures for the intervention are listed by outcome in @tbl-outcomes-summary and are estimated from the mixed-effects models in @sec-analysis-methods. These models provide cluster-specific (conditional) estimates of the intervention effect for patient-level outcomes; they are not marginal (population-averaged) estimates. The target of inference is the participant-level average among eligible patients in participating clusters, rather than an equal-weight average of cluster-level summaries. The types of effect measures are as follows:
...
## Analysis methods {#sec-analysis-methods-overview}
...
Time-adjusted within-cluster correlations and variance components will be presented as a supplementary table, as specified under @sec-correlations. See @tbl-correlation-parameters.
