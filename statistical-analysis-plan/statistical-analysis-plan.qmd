...
An observation is one presentation by an eligible patient to the emergency department of a participating cluster; see @sec-eligibility-criteria. Each observation belongs to the cluster the patient presented to and to the period in which the patient arrived at the emergency department.

Every randomised cluster contributes observations, and a cluster that leaves the trial early contributes the periods it completed. Observations from a cluster’s transition period are excluded from all analyses.

An observation is classified as exposed to the intervention if the patient arrived after the date on which the cluster was scheduled to transition, and as standard care if the patient arrived before that date. Exposure does not depend on whether a trained physician delivered the care, or on when the cluster was in fact trained; see @sec-adherence-and-protocol-deviations. If a cluster transitions more than four weeks from its scheduled date, we will repeat the primary analysis with exposure defined by the actual date of transition; see @sec-exposure-defined-by-the-actual-date-of-transition.
...
- **Primary analysis set.** All observations. This set is used for the primary outcome and for the secondary outcomes collected with the main stepped-wedge design.
- **ICU admissions, primary analysis set.** The observations for patients admitted to intensive care.
- **Alive at 30 days, outcome relevant.** The observations for patients who were alive 30 days after arrival at the emergency department and for whom return to work is relevant, determined by the patient’s work status before the injury.
- **Alive at 3 months, outcome relevant.** The same restriction applied three months after arrival at the emergency department.
