...
The primary outcome is in-hospital mortality within 30 days of arrival at the emergency department and will be analysed as a dichotomous variable. We will estimate the primary intervention effect as the OR of death between the ATLS^®^ and standard care arms, with an OR \< 1 indicating lower odds of death in the ATLS^®^ arm compared to the standard care arm and vice versa.

### Primary analysis

We will use a mixed-effects binomial regression model to estimate the effect of ATLS^®^ on mortality. The model will include fixed effects for period, modelled as a categorical variable, and a fixed effect for intervention exposure. Clustering will be accounted for by including a random intercept for cluster and a random cluster-by-period effect, both assumed to follow a normal distribution. The full model is specified in @eq-primary-model.
...
The primary analysis model will be fitted using residual pseudo-likelihood estimation based on linearisation with subject-specific expansion (RSPL), due to its computational stability and robust performance in small-sample settings.
...
$$
g{Pr(Y_bkti = 1)} = μ + β_bt + θ X_bkt + α_bk + γ_bkt
$$
...
- $\beta_{bt}$: the fixed effect of period $t$ in batch $b$, accounting for a separate period effect for each batch, resulting in a total of $\texttt{r batches} \times (\texttt{total.months} - 1)$ period effects.[^low-number-of-participants]
...
In all models, the link function $g(\cdot)$ will be specified as either the logit link or the identity link.

When the logit link is used, the intervention effect $\theta$ will be interpreted as a log-odds ratio, and results will be presented as an odds ratio for mortality with a corresponding 95% confidence interval, using the standard care arm as the reference.

When the identity link is used, $\theta$ will be interpreted as an absolute risk difference, representing the difference in the probability of death between the ATLS^®^ and standard care arms. Results will be presented as a risk difference with a 95% confidence interval.

If the model with the logit link converges but the corresponding model with the identity link does not, results will be reported using the logit link only.

We will not adjust the primary analysis models for cluster size, as randomisation is balanced with respect to cluster size.

### Plan to handle model convergence issues

If the primary analysis model does not converge or is numerically unstable, we will fit a prespecified sequence of progressively simpler models. This sequence will retain the intervention exposure term, and time will be adjusted for in all models.

The following sequence will be applied:

1. **Simplified random effects structure**: models including only a random intercept for cluster, thereby removing the random cluster-by-period effect. The simplified model is specified in @eq-simplified-random-effects-model.

$$
g\{\Pr(Y_{bkti} = 1)\}
= \mu + \beta_{bt} + \theta X_{bkt} + \alpha_{bk}
$$
{#eq-simplified-random-effects-model}

2. **Model with shared period effects**: a model in which period effects are shared across batches, with time adjusted for using a common period effect or calendar time modelled directly (e.g., as a continuous covariate or using splines). This alternative parametrisation of time may improve model stability, particularly as the last batch begins nearly two years after the first. The model is specified in @eq-shared-period-model.
...
3. **Combined simplified model**: a model that includes both a simplified random effects structure (random intercept for cluster only) and shared period effects. This represents the most parsimonious mixed-effects model considered acceptable for the primary analysis. The model is specified in @eq-simplified-random-effects-model-and-shared-period-model.

$$
g\{\Pr(Y_{bkti} = 1)\}
= \mu + \beta_{t} + \theta X_{bkt} + \alpha_{bk}
$$
{#eq-simplified-random-effects-model-and-shared-period-model}

If the binomial model with the logit link converges but the corresponding model with the identity link does not, then we will report only an OR. 

The main analysis will be based on the most complex model in this sequence that converges, and this model will then be subject to sensitivity analyses.

### Sensitivity analyses

The sensitivity analyses will be conducted to assess the robustness of the main analysis results to different model specifications. Each sensitivity analysis will be operationalised using two separate models, one with the logit link and one with the identity link. We will first explore more complex correlation structures. We will then model time using a spline function. Finally, we will conduct a fully adjusted covariate analysis.
