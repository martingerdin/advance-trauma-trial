...
At the cluster level, late or early transition relative to the scheduled date, incomplete delivery of ATLS® training, and cluster dropout are treated as protocol or adherence issues (see @sec-adherence-and-protocol-deviations) rather than as outcome-specific intercurrent events; exposure for analysis remains as defined under @sec-analysis-sets, with a sensitivity analysis using the actual transition date when the deviation exceeds four weeks.
