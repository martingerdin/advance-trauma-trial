...
Process adherence to ATLS® principles during initial resuscitation is a nested-staircase secondary outcome and is defined and analysed in @sec-outcomes-overview and @sec-analysis-of-secondary-outcomes; it is not used to exclude patients from the primary analysis.
=======
We define adherence to the intervention at the cluster and individual levels. It is distinct from the secondary outcome adherence to ATLS^®^ principles during resuscitation, which we define under @sec-outcomes, analyse under @sec-analysis-of-secondary-outcomes, and do not use to exclude patients from the primary analysis.
...
<<<<<<< HEAD
We will use CONSORT diagrams to display the flow of clusters and patients through the main batched stepped-wedge trial, reported as two figures in the main text, in line with recent batched stepped-wedge practice [@escp_eagle_2024]: a cluster-level diagram (@fig-consort-clusters) and a patient-level diagram (@fig-consort-patients). For each sequence, the diagrams will report numbers randomly assigned or included, losses and exclusions after randomisation (with reasons), and numbers analysed for the primary outcome. The patient diagram will also summarise flow before and after ATLS® training. Flow for nested-staircase secondary outcomes (adherence to ATLS® principles, EQ-5D-5L, and WHODAS 2.0) will be reported in a separate figure in the supplementary material (@fig-consort-nested-staircase), showing patients in the staircase periods, losses and exclusions, and numbers analysed, by sequence, before and after ATLS® training, and overall. We will present cluster-level summaries of the intervention effect. We will report the study according to the CONSORT guidelines for stepped-wedge randomised trials [@Hemming2018].
=======
...
We will describe cluster characteristics including state and size using frequencies and percentages for discrete variables and medians and interquartile ranges (Q1-Q3) for continuous variables. State categories depend on the final set of participating clusters. We will present these characteristics in the main text. See @tbl-cluster-characteristics for details.
=======
...
<!-- sap-revision-todo J7: non-floating longtable so placement stays in document order. -->
<<<<<<< HEAD
We will describe patient characteristics among all eligible patients over the entire trial period, stratified by before and after ATLS training and overall, using frequencies and percentages for discrete variables and medians and interquartile ranges (Q1-Q3) for continuous variables. Missing values will be reported for each characteristic. We will not adjust for clustering when presenting baseline characteristics (i.e. non-outcome variables). We will present key characteristics in the main text, and present all characteristics in the supplementary material. See @tbl-patient-characteristics for key characteristics and @tbl-patient-characteristics-all for all characteristics.
...
A summary of all outcomes, analysis sets, effect measures, and handling of intercurrent events is provided in @tbl-outcomes-summary. The strategies named in the last column of that table are defined under @sec-strategies-for-handling-intercurrent-events. 
=======
...
#| tbl-cap: "Summary of outcomes, analysis sets, effect measures, and handling of intercurrent events. The strategies named in the last column are defined in the section Strategies for handling intercurrent events."
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
<<<<<<< HEAD
Intercurrent events are events occurring after intervention exposure that affect interpretation or occurrence of an outcome. Anticipated events and the strategy used for each outcome are summarised in @tbl-outcomes-summary. Frequencies will be summarised for each outcome as counts and percentages of patients with the event before assessment of that outcome, stratified by before and after ATLS training and overall. We will not adjust for clustering in these summaries. Denominators are the primary analysis set or the nested sample, before the outcome-specific restrictions under @sec-analysis-sets, so that events that remove a patient from an analysis set are still counted. We will present these summaries in the supplementary material. See @tbl-intercurrent-events.

At the cluster level, late or early transition relative to the scheduled date, incomplete delivery of ATLS® training, and cluster dropout are treated as protocol or adherence issues, as described under @sec-adherence-and-protocol-deviations, rather than as outcome-specific intercurrent events; exposure for analysis remains as defined under @sec-analysis-sets.
=======
...
Return to work is further restricted to patients for whom the outcome is relevant, as defined under @sec-analysis-sets; that restriction defines the analysis set and is not an intercurrent event.
...
The strategies to handle intercurrent events are defined as follows, in the order in which they first appear in @tbl-outcomes-summary. Where two strategies are listed for an outcome, the second applies to any event not covered by the first.

- **Missing data.** The outcome is not observed and no value is constructed for it. Such observations are excluded from the analysis of that outcome under the available-case approach, or imputed under the multiple imputation approach, as specified under @sec-treatment-of-missing-data.
- **Treatment policy (any setting).** The outcome is counted as recorded irrespective of where care was delivered after the patient left the randomised cluster. All-cause mortality therefore counts deaths in any setting, including after transfer or discharge.
- **Observed stay truncated by death or transfer.** The length of stay is the completed duration until the patient leaves the unit, including when the patient leaves by dying or being transferred. These stays enter the analysis at their observed value and are not treated as censored.
...
- **Available-case in the observed nested sample.** The outcome is analysed among the resuscitations that were fully observed. Unobserved and incompletely observed resuscitations are excluded.
...
A population is the group of clusters and patients to which an estimated effect applies. An analysis set is the subset of a population whose observations enter a given analysis, as defined under @sec-analysis-sets. The treatment effects estimated in this plan are defined for the trial’s eligible clusters and eligible patient participants, as specified under @sec-eligibility-criteria.
=======
...
<<<<<<< HEAD
=======
We specify which patients from each target population enter a given analysis under @sec-analysis-sets.
...
An observation is one presentation by an eligible patient to the emergency department of a participating cluster, see @sec-eligibility-criteria. Each observation belongs to the cluster the patient presented to and to the period in which the patient arrived at the emergency department. Every randomised cluster contributes observations, and a cluster that leaves the trial early contributes the periods it completed. Observations from a cluster’s transition period are excluded from all analyses.

An observation is classified as exposed to the intervention if the patient arrived after the period on which the cluster was scheduled to transition, and as standard care if the patient arrived before that period. Exposure does not depend on whether a trained physician delivered the care, or on when the cluster was in fact trained, see @sec-adherence-and-protocol-deviations. If a cluster transitions more than four weeks from its scheduled date, we will repeat the primary analysis with exposure defined by the actual date of transition, see @sec-exposure-defined-by-the-actual-date-of-transition.

The observations for the patients selected for nested staircase data collection form the nested sample. These patients are drawn by stratified random sampling by shift during the three months before and the three months after the transition period, see @sec-nested-staircase-design-and-secondary-outcomes.

@tbl-outcomes-summary names an analysis set for each outcome. The sets are defined as follows, in the order in which they first appear in that table.

- **Primary analysis set.** All observations, conditioned on the missing data strategy used. This set is used for the primary outcome and for the secondary outcomes collected with the main stepped-wedge design.
- **ICU admissions.** The observations for patients in the primary analysis set who were admitted to intensive care.
- **Alive at 30 days and working at the time of inclusion.** The observations for patients who were alive 30 days after arrival at the emergency department and who were working at the time of inclusion. Working means paid or unpaid work, studying or keeping house.
- **Alive at 3 months and working at the time of inclusion.** The same restriction applied three months after arrival at the emergency department.
- **Observed nested sample.** The observations in the nested sample for which the resuscitation was observed in full.
- **Alive, nested sample.** The observations in the nested sample for patients who were alive when the outcome was assessed.
- **Alive at 30 days, nested sample.** The observations in the nested sample for patients who were alive 30 days after arrival at the emergency department.
- **Alive at 3 months, nested sample.** The observations in the nested sample for patients who were alive three months after arrival at the emergency department.

An analysis includes the observations in the analysis set for that outcome whose outcome value is observed. Under multiple imputation, observations with a missing outcome are retained and the value is imputed, see @sec-treatment-of-missing-data. Adjusted analyses under the available-case approach include only observations with complete data on the adjustment covariates.
=======
...
Primary and secondary outcomes will be analysed with mixed-effects regression models that account for clustering at the hospital level and target cluster-specific intervention effects. The link function and outcome distribution are matched to the data type of each outcome, and the model for each outcome is specified in the subsections below.
...
Unless stated otherwise, the estimation and reporting specifications below apply to all analyses in this section. Estimation will be by maximum likelihood, with a Laplace approximation used to integrate out the random effects. The specific R package and numerical implementation are not prespecified, see @sec-statistical-software.
...
Unless otherwise stated, secondary outcomes will be analysed using the model specification selected for the primary analysis under @sec-analysis-of-the-primary-outcome, with the link function and outcome distribution adapted to the scale of the outcome. The period parametrisation and random effects structure of that specification are carried over, so the sequence of models of decreasing complexity is not traversed again for each secondary outcome. If the specification selected for the primary analysis does not converge for a secondary outcome, we will simplify the structure for that outcome by continuing down the same sequence, and we will report which specification was used for each outcome. For outcomes collected within the nested staircase design, period effects are defined on the global study timeline rather than separately within each batch, as described in the subsections below; the random effects structure still follows the primary analysis.

Effect measures are odds ratios and absolute risk differences for binary outcomes, rate ratios for count (length-of-stay) outcomes, cumulative odds ratios for ordinal outcomes, mean differences for continuous outcomes, and a logit-scale difference for the adherence proportion. These models provide cluster-specific estimates of the intervention effect for patient-level outcomes. For cumulative logit models, diagnostics will also assess the proportional-odds assumption.
...
These outcomes will be analysed as dichotomous variables using the mixed-effects binomial model selected for the primary analysis, with the logit and identity links, to estimate odds ratios and absolute risk differences.
=======
...
The model is a mixed-effects negative binomial regression with a log link, including a fixed effect for intervention exposure and the period parametrisation and random effects structure selected for the primary analysis. @eq-length-of-stay-model shows this model with batch-specific categorical period effects, a random intercept for cluster, and a random cluster-by-period effect; if a simpler structure was selected for the primary analysis, that structure is used here instead. If the model does not converge for this outcome, we will simplify it further by continuing down the same sequence. The intervention effect $\theta$ is the log rate ratio for ED length of stay associated with ATLS® exposure compared with standard care; we will also report the corresponding multiplicative effect on the mean count (rate ratio).
=======
...
Length of hospital stay will be analysed as a count outcome: the number of days from arrival at the emergency department to hospital discharge (or death in hospital). The analysis uses the same negative binomial specification as for ED length of stay (@eq-length-of-stay-model), with the same handling of death and transfer.
=======
### Length of hospital stay {#sec-length-of-hospital-stay}
...
<<<<<<< HEAD
<!-- github-issue-69: the actual-transition-date sensitivity analysis is referenced from three sections and has a row in the results shell, but had no subsection of its own. -->
#### Exposure defined by the actual date of transition {#sec-exposure-defined-by-the-actual-date-of-transition}
If a cluster transitions more than four weeks from its scheduled date, we will redefine exposure for that cluster using the actual date of transition, excluding the month in which the transition took place, and repeat the primary analysis, see @sec-analysis-sets.
...
<<<<<<< HEAD
<!-- github-issue-72: state whether the shell is intended for the main text or the supplementary material. -->
Safety events, as defined in the trial protocol, will be summarised as counts and percentages of patients with at least one recorded event, stratified by before and after ATLS training and overall. We will not adjust for clustering in these summaries. We will present these summaries in the main text. See @tbl-safety-events for details.
=======
...
The captions in this section state whether each table or figure is intended for the main text or the supplementary material of the trial report.
...
<<<<<<< HEAD
#| fig.cap: "Cluster-level CONSORT diagram shell. Placeholders cover clusters assessed and randomised, losses and exclusions after randomisation with reasons, and clusters included in the primary analysis, by sequence and overall. The flowchart summarises data across all batches 1-6. Intended for the main text."
=======
...
<<<<<<< HEAD
#| fig.cap: "Patient-level CONSORT diagram shell. Placeholders cover patients entered, losses and exclusions from the primary analysis with reasons, and patients analysed, by sequence, before and after ATLS training, and overall. The flowchart summarises data across all batches 1-6. Intended for the main text."
=======
...
#| fig.cap: "Forest plot of subgroup-specific intervention effects on in-hospital mortality within 30 days. Estimates and confidence intervals are placeholders to be completed at analysis. Intended for the main text."
#| fig.align: "center"
#| fig-pos: H
#| echo: false
#| message: false
