...
#| fig.cap: "Nested staircase design. The design includes a random subset of patients presenting during the three months preceding the transition phase, and the three months following the transition phase. Intended for the supplementary material."
#| fig.align: "center"
#| message: FALSE
#| warning: FALSE
...
At the individual level, exposure to the intervention is defined by the cluster’s scheduled transition date, as specified under @sec-analysis-sets. Patients are classified according to the period of admission, and the primary analysis includes eligible patients irrespective of whether care was delivered by a trained physician. If a cluster’s actual transition date differs from the scheduled date by more than four weeks, exposure will be redefined using the actual transition date in a sensitivity analysis; see @sec-analysis-sets.
...
A summary of all outcomes, analysis sets, effect measures, and handling of intercurrent events is provided in @tbl-outcomes-summary, which is intended for the supplementary material. The strategies named in the last column of that table are defined under @sec-strategies-for-handling-intercurrent-events. 
...
#| tbl-cap: "Summary of outcomes, analysis sets, effect measures, and handling of intercurrent events. The strategies named in the last column are defined in the section Strategies for handling intercurrent events. Intended for the supplementary material."
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
The strategies named in the last column of @tbl-outcomes-summary are defined as follows, in the order in which they first appear in that table. Where two strategies are listed for an outcome, the second applies to any event not covered by the first.
...
<!-- github-issue-69: open with the population/analysis-set relation before the operational rules. -->
Each analysis set is a subset of the population for the outcome being analysed, as defined under @sec-population. This section specifies how that subset is formed.
...
<!-- github-issue-69: state the membership rules instead of only cross-referencing other sections. -->
Three further rules define the analysis set for each analysis. First, only patients with observed data on the outcome being analysed are included. Second, the outcome-specific restrictions listed in @tbl-outcomes-summary are applied, such as admission to intensive care for length of intensive care unit stay, and being alive at the relevant follow-up for quality of life, disability, and return to work. Third, adjusted analyses under the available-case approach are further restricted to patients with complete data on the adjustment covariates, as described in @sec-treatment-of-missing-data.
...
<!-- github-issue-72: captions state the intended destination of every table and figure. -->
Every table and figure caption in this plan states whether the table or figure is intended for the main text or the supplementary material of the trial report. Intervention-effect estimates for the primary and secondary outcomes will be reported with 95% CIs and p-values, using the effect measures specified for each outcome. We will present key outcome analyses in the main text, and present all outcome analyses in the supplementary material. See @tbl-outcomes-analysis-results for key analyses and @tbl-outcomes-analysis-results-all for all analyses.
