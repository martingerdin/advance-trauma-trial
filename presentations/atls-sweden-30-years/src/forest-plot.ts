...
export function createForestPlotSvg(data: MetaAnalysisData = metaAnalysis): SVGSVGElement {
  ...
  data.studies.forEach((study, i) => {
    ...
    const row = svgEl("g", { class: "forest-row", transform: `translate(0, 0)` });
    ...
    const xHi = Math.min(logToX(Math.log(study.ciUpper), data.logScaleXlim, forestX), forestX + FOREST_WIDTH);
    ...
    row.append(name, n, whisker, square, effect);
    svg.appendChild(row);
  });
  ...
  const pooledName = svgEl("text", { x: PAD_LEFT, y: pooledY + 4, class: "forest-study forest-study--pooled" });
  ...
  const pHi = Math.min(logToX(Math.log(data.pooled.ciUpper), data.logScaleXlim, forestX), forestX + FOREST_WIDTH);
  const diamondH = 7;
  ...
  pooledRow.append(pooledName, diamond, pooledEffect);
