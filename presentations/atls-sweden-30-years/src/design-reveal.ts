...
export function startDesignReveal(
  ...
  const animateSite = async (token: number, animateIn: boolean) => {
    ...
    await morphCamera("site", token, animateIn ? 1.0 : 0);
    hideAllRows();
    ...
    await scrubTimeline(0, shortEnd, token, 4.0);
  };
  ...
  const animateBatch = async (token: number, animateIn: boolean) => {
    ...
    await clearPlot(token, animateIn);
    await morphCamera("batch", token, animateIn ? 1.2 : 0);
    ...
    await scrubTimeline(0, shortEnd, token, 4.2);
  };
  ...
  const animateFull = async (token: number, animateIn: boolean) => {
    ...
    await clearPlot(token, animateIn);
    await morphCamera("full", token, animateIn ? 1.5 : 0);
    ...
    await scrubTimeline(0, fullEnd, token, 5.6);
  };
